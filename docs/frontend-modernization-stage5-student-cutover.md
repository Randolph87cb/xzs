...
- 旧学生端 Vue 2 源码目录在阶段 5 时仍保留。它已不再是默认生产构建入口，但后续仍需要删除或替换该目录，避免误用。
- 管理端在阶段 5 时仍未迁移，继续使用旧管理端 Vue 2 + Vue CLI 构建。
- 题目渲染共享 chunk 仍偏大，后续阶段可继续拆分 KaTeX、Markdown、DOMPurify 和代码高亮依赖。

最终状态更新：Vue 3 + Vite 覆盖迁移已完成，旧 Vue 2 Web 源码目录已删除，学生端和管理端默认生产构建统一来自 `frontend/`。

## 阶段 5 结论

学生端默认生产构建、后端 static 同步、后端 jar 内置 `/student` 入口和严格真实业务截图验证均已切换到 Vue 3 + Vite。阶段 5 的生产入口切换完成；后续阶段已继续完成管理端迁移和旧工程清理。
