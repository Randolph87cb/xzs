# Fly.io 部署与历史 Fly Postgres 说明

当前状态：生产数据库已迁移到 Neon PostgreSQL。本文保留 Fly Postgres 冷启动方案作为历史方案和回滚参考；新的 Fly Web App 部署默认只配置 Neon 数据库 secret，不再创建或挂载 Fly Postgres 主库。

## 架构

- Fly App：运行 Spring Boot jar，端口 `8000`，内置管理端和学生端 Vue 3 + Vite 静态资源。
- Neon PostgreSQL：当前业务主库。生产环境使用 Neon `production` branch；本地构建服务测试和测试环境使用 Neon `test` branch。
...
当前部署目标是 Fly Web App 低成本冷启动 + Neon PostgreSQL：
...
Fly Web App 使用 Neon 原始 URL 形式配置数据库。连接串包含用户名和密码时，不需要额外设置 `SPRING_DATASOURCE_USERNAME` 和 `SPRING_DATASOURCE_PASSWORD`：

```powershell
fly secrets set `
  SPRING_DATASOURCE_URL="postgresql://<user>:<password>@<production-branch-host>/<database>?sslmode=require&channel_binding=require" `
...
本地构建服务测试使用 Neon `test` branch。复制 `.env.neon-test.example` 为 `.env.neon-test` 后填入 `test` branch 连接串：
...
- 生产发布和线上 Fly App 连接 `production` branch。
- 本地直接启动 Java 服务、构建后验收和测试环境连接 `test` branch。
...
停机后再次访问应用 URL，Fly 会按需启动 Web Machine。当前数据库在 Neon，不需要停止 Fly Postgres Machine。历史 Fly Postgres App 如需保留冷备状态，可保持 stopped；销毁 App 或 Volume 前必须再次确认。
...
- Neon PostgreSQL：当前主库，用于账号、题目、试卷、答卷、成绩、消息等业务数据。
