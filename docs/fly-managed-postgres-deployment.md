...
- 现有 Fly Postgres App：`xzs-pg-cb867393296`
- 现有业务库：`xzs_cb867393296`

`gesp-csp-quiz` 直接对应 GESP/CSP 训练场景，便于记忆和分享。数据库名称暂时沿用历史部署名，避免迁移已有数据；如后续需要完全统一命名，再单独做数据库备份、恢复和连接串切换。
