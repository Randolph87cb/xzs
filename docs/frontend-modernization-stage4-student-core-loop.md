...
阶段 4 的完整目标是迁移学生端核心业务。本次提交先完成一个可验证的核心子闭环：

```text
登录 -> 试卷中心 -> 取卷答题 -> 交卷入口 -> 考试记录
```

同时补充智能训练入口。错题本、个人中心、消息中心、查看/批改试卷详情页还未迁移。
...
| 主 JS | `1043.79 kB`, gzip `340.97 kB` |
| 答题页 JS | `747.31 kB`, gzip `237.39 kB` |
| 答题页 CSS | `32.98 kB`, gzip `9.26 kB` |
...
学生端核心子闭环已经具备可运行基础：可登录、可拉取试卷列表、可进入答题页、题目内容使用新 renderer 渲染、可查看考试记录、可创建智能训练卷。该子闭环还不是完整阶段 4，不能进入阶段 5 覆盖切换。
