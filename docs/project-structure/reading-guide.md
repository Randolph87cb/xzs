...
1. Web 路由：管理端读 `source/vue/xzs-admin/src/router.js`，学生 Web 端读 `frontend/apps/student/src/router/index.ts`。
2. 页面组件：对应 `views` 目录下的 `.vue` 文件。
3. 前端 API：管理端读对应 `src/api/*.js`，学生 Web 端读 `frontend/packages/api-client/src/*.ts`。
...
- 管理端：先读 `source/vue/xzs-admin/src/router.js`，再进入对应 `views` 和 `api` 文件，最后查 `/api/admin/...` 的后端 Controller。
