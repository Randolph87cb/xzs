...
- 图片上传和 UEditor 公式插件弹窗尚未做浏览器交互级验证；当前验证覆盖 `$...$` Markdown/KaTeX 渲染和 UEditor 内容保存。
- 管理端 Vue 3 仍未接管后端 `/admin` static 生产入口。

## 阶段 7 结论

管理端题库的 Vue 3 最小可用闭环已经跑通，且补上了会导致 dev/HMR 白屏的 UEditor 宿主实现问题。下一阶段应继续扩展题型覆盖和管理端剩余模块，然后进入管理端生产候选和 `/admin` 发布入口切换。
