...
管理端 UEditor 是迁移风险最高的单点之一。它依赖全局脚本、iframe 弹窗和公式插件；后端上传接口已关闭。Vue 3 迁移有两条路线：

- 保守路线：保留 UEditor 静态资源，重写 Vue 3 wrapper，继续动态加载 `window.UE`。
- 长期路线：替换为 Vue 3 生态富文本编辑器，并迁移公式、历史图片外链展示、HTML 回显和历史题目内容兼容。
...
- 验证题目编辑、选项编辑、解析编辑、公式插件、历史图片外链展示和内容回显。
