...
| 环境 | 编排方式 | 镜像来源 | 数据库 |
| --- | --- | --- | --- |
| 树莓派生产 | `docker compose` | 阿里云 ACR `randolph87/xzs` | Neon `production` branch |
...
docker buildx build --platform linux/arm64 `
  -t "crpi-s5bag0a5r8vcgncq.cn-hangzhou.personal.cr.aliyuncs.com/randolph87/xzs:$tag" `
...
- 树莓派生产环境从 `/opt/apps/xzs/.env` 读取生产 secret，模板为 `docker/.env.production.example`。
