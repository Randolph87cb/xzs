...
- 允许自动化辅助：可以用 SQL/脚本查重复、查引用、生成待审清单、统计质量指标、同步已人工确认的 Markdown 到 Fly，但不能让自动化决定解析内容。
- 源题库优先：最终确认后的解析必须回写 `docs/question-bank/GESP`，Fly 数据库只作为线上运行副本。
...
- 修改方案：
  ...
  6. 同步线上：只把已经人工确认的源文件变更同步到 Fly。
- 影响范围：`docs/question-bank/GESP/**/选择题.md`、`docs/question-bank/GESP/**/判断题.md`、后台审核记录和 Fly `t_text_content.content`。
...
- 判断：如果只是同步已经人工确认的解析，优先用迁移 SQL 更新匹配题目的内容；如果新增题目、修题目元数据或清理重复题，再使用单独审阅过的 SQL。同步脚本不能生成解析，只能搬运人工确认内容。
...
9. 备份并同步 Fly：执行迁移 SQL 后再次跑只读统计。
10. 补管理端质量队列：在批量修复开始前或第一批之后实现，便于后续人工二审。
