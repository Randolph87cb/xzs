...
- 待确认：Fly 远端生产库的实时问题数量。本次排查中 `flyctl` 已登录，但 Fly API 多次返回 EOF，无法完成远端只读统计；不能把本地数量直接等同于生产库数量。

## 结论

推荐采用“先建质量清单，再修 Markdown 源题库，最后同步 Fly”的路线。少量紧急题目可以直接通过管理端审核页改远端库，但批量修复必须回写 `docs/question-bank/GESP`，否则后续重新导入会覆盖生产库中的人工修复。

## 需求拆解

### 1. 远端库现状统计

- 当前现状：Fly 部署文档记录 Web App 为 `gesp-csp-quiz`，Postgres App 为 `xzs-pg-cb867393296`。本机有 `flyctl` 且已登录，但没有本地 `psql` 命令；项目导入脚本支持通过 Docker `postgres:17` 镜像执行远端 `psql`。
- 判断：远端统计应先只读执行，避免直接导入或改库。统计口径要和本地脚本一致，至少覆盖 `暂无解析`、短解析、模板化短语、审核轮次和按批次分布。
- 修改方案：新增或临时执行一份只读 SQL，产出远端质量报表。建议先不做写入。
- 影响范围：只读查询 `t_question`、`t_text_content`、`t_question_review_record`。
- 验证方案：确认统计 SQL 返回总题量、GESP 题量、缺解析题数、短解析题数、模板命中题数、受影响题数，并抽样 20 道题核对题干、答案、解析内容。
...
1. 远端只读统计：连接 Fly Postgres，保存当前质量基线和样本清单。
2. 修复优先级排序：先处理 `暂无解析` 和短解析，再处理 2024 全年模板化解析，最后处理 2025 选择题模板化解析。
3. 批次修复 Markdown：每次只处理一个年月或一个级别，降低审核压力。
4. 本地质量检查：运行导入脚本 `-QualityCheck`，确认问题数下降并抽查样本。
5. 生成并审阅迁移 SQL：使用 `-MigrationSqlOnly`，确认匹配和待更新数量。
6. 备份并同步 Fly：执行迁移 SQL 后再次跑只读统计。
7. 补管理端质量队列：在批量修复开始前或第一批之后实现，便于后续人工二审。

## 风险与待确认

- Fly API 本次多次 EOF，远端实时统计仍待确认。
- 本地源题库 `暂无解析` 为 0，但远端库可能存在历史导入、后台新增或未同步题，必须以远端 SQL 为最终清单。
